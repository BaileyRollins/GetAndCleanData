...
The script assumes the data is in the same folder structure as the zip file, but in the working directory.  
A variable "datapath" may be set to use an alternate directory, if desired.
...
* Since the activity data is encoded, a lookup set (activityLookup) is created to merge later for descriptive activity names
...
* The observations data table is updated with descriptive names as describes in the CodeBook
* The activity names (activityLookup) are merged into the observations data set and "Activity_Id" is removed
